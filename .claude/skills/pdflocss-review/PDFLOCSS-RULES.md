...
- 単語はハイフンでつなぐ(**ケバブケースのみ**。キャメルケースは使わない — 弊社独自ルール)。
- **連番(弊社独自ルール)**: 1番目には連番を付与せず、2番目から付与する(`item`、`item2`、`item3`...)。ゼロパディングはしない(`item2`は○、`item02`は✕)。連番の前にハイフンは入れない(`item2`は○、`item-2`は✕)。ただし値そのものを表す数字(例: `opacity-06`)はハイフンを入れる(これは連番ではなく値の一部)。PDFLOCSS本来のゼロパディング2桁ルールを置き換える。
- レイヤー接頭辞は本家FLOCSSと同じ: `l-`(layout) / `c-`(component) / `p-`(project) / `u-`(utility)。

## レイヤー別ルール

### Foundation(接頭辞なし)
初期化・変数・ベーススタイル。ファイル分割の目安: `_font.scss`(フォント定義) / `_mixin.scss`(`@mixin`とメディアクエリ) / `_function.scss`(`@function`) / `_reset.scss`(リセットCSSそのまま、**自分で編集しない**) / `_base.scss`(リセットしきれなかった分+ベーススタイル)。
- **変数(弊社独自ルール)**: 色・サイズ等の変数は`_color.scss`/`_variable.scss`のようなSassファイルではなく、**`tokens`ディレクトリにCSS変数(カスタムプロパティ、`--c-primary`等)として集約する**(Sassの`$variable`は使わない)。出典: [website-starter-kit — tokens](https://github.com/sonicmoov/website-starter-kit/tree/main/src/styles/foundation/tokens)。命名はできるだけセマンティック(`--c-primary`)を優先し、無理なら色名(`--c-gray`)にフォールバックする方針自体はPDFLOCSS本来の考え方を踏襲する。
...
- **PDFLOCSS本(書籍)から取ったディレクトリ構成・ファイル名の規則(`foundation/layout/object/...`というディレクトリ名、`_[ページ名].scss`等のファイル名)は、website-starter-kit準拠のプロジェクトにのみ厳密に適用する**(弊社独自ルール)。Next.js/Nuxt/Rails/Astro(website-starter-kit以外)はいずれも物理ディレクトリ名・ファイル名の一致を問わず、**フレームワーク準拠のディレクトリ構成・ファイル名でよい**。判定するのは「役割の分離(初期化/構造/再利用部品/ページ固有/微調整)が命名やファイル構成のどこかで区別できているか」。website-starter-kit準拠かどうかの判定方法・Astro固有の詳細は下記「Astro + website-starter-kit(弊社独自ルール)」参照。
...
- **should-fix**: 命名規則の逸脱(キャメルケース混在、連番のゼロパディング/1番目への付与/ハイフン前置、Element2階層化等)、rule of three違反のコンポーネント化/コピペ判断ミス、状態操作を`.is-○○`のようなclassで行っている(「JS連携(弊社独自ルール)」の属性優先順位に従っていない)、色・変数をSassの`$variable`で定義していてCSS変数(`tokens`)になっていない。
